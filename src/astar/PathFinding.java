...
public class PathFinding {
     ...
     * @return
     */
    public static List<Vector2D> findPath(Grid grid, Vector2D startPos, Vector2D targetPos, boolean allowDiagonals) {
        // Find path
        List<Node> pathInNodes = findPathNodes(grid, startPos, targetPos, allowDiagonals);

        // Convert to a list of points and return
        List<Vector2D> pathInPoints = new ArrayList<Vector2D>();

        if (pathInNodes != null)
            for (Node node : pathInNodes)
                pathInPoints.add(new Vector2D(node.x, node.y));
        return pathInPoints;
    }
     ...
     */
    private static List<Node> findPathNodes(Grid grid, Vector2D startPos, Vector2D targetPos, boolean allowDiagonals) {
